...
面向分布式文件与对象存储，补充这些攻击面与控制：

- **多租户隔离**——在命名空间、配额与 IO 路径上强制租户间硬边界；喧闹或恶意的租户不得读取、饿死或损坏另一租户
- **加密**——静态（每租户密钥、信封加密）与传输中（RDMA/TCP、节点间复制）；定义密钥生命周期与轮换
- **访问控制**——协调 S3 桶/对象策略与 POSIX/ACL 语义；对客户端和内部服务都遵循最小权限
- **密钥与凭据管理**——集成 KMS，轮换时无需重加密全部数据，密钥不落盘、不进日志
- **完整性**——端到端校验和作为安全控制（检测静默损坏与篡改），而不只是可靠性特性
- **控制面加固**——对节点加入、管理、再平衡操作做认证与授权；恶意节点不得加入集群或接收副本
